...
<html>
	...
	<body>
		<!-- start 当前位置 -->
		<div id="tab_position">
			<span class="tab_position_b">日数据分析</span>
		</div>
		<!-- end 当前位置 -->
		...
		<s:form id="queryForm" action="dayList"
			namespace="/towertilt/analysis" method="post" theme="simple">
									...
									<table width="100%" cellspacing="1" class="tab">
										...
										<tr>
											...
											<td colspan="2" class="table_td_left">
												<s:checkbox name="showInclination" />
												<span>倾斜度</span>
												<s:checkbox name="showGradientAlongLines" />
												<span>顺线倾斜度</span>
												<s:checkbox name="showLateralTilt" />
												<span>横向倾斜度</span>
												<s:checkbox name="showAngle_x" />
												<span>顺线倾斜角</span>
												<s:checkbox name="showAngle_y" />
												<span>横向倾斜角</span> &nbsp;&nbsp;&nbsp;&nbsp;
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td class="third_line_center">
									<s:submit value="查 询" cssClass="btn2" action="dayList" />
									&nbsp;&nbsp;
					...
					<tr>
						<td colspan="4" class="table_td_left">
							<s:url action="dayChart" namespace="/towertilt/analysis"
								...
								<s:param name="showGradientAlongLines"
									value="showGradientAlongLines" />
						...
						<td height="30">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td class="first_line_center">
										<div class="div_right">
											<!-- <s:url action="dayChart" namespace="/analysis/towertilt" var="chartUrl">
										<s:param name="sensorId" value="sensorId" />
										<s:param name="beginTime">
											<s:date name="beginTime" format="yyyy-MM-dd" />
										</s:param>
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td class="third_line_center">
										...
										<yixin:page url="/towertilt/analysis/dayList.jspx"
											page="${page}" id="towerTiltDayAnalysis_page">
											...
											<yixin:hidden name="showInclination"
												value="${requestScope['showInclination']}" />
											<yixin:hidden name="showGradientAlongLines"
												value="${requestScope['showGradientAlongLines']}" />
											<yixin:hidden name="showLateralTilt"
												value="${requestScope['showLateralTilt']}" />
											<yixin:hidden name="showAngle_x"
												value="${requestScope['showAngle_x']}" />
											<yixin:hidden name="showAngle_y"
												value="${requestScope['showAngle_y']}" />
										</yixin:page>
									</td>
									<td class="third_line_right">
										&nbsp;
