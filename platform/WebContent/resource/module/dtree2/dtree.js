...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if(this.config.check==true){
			...
			str+= '<img checked=0  name="' +node.id + '"  src="'+this.icon.iconUnCheckAll+'" value="'+node.id+'" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
			str+='<input type="checkbox" style="display:none" name="'+this.config.mycheckboxName+'"   value="'+node.id+'" id="ccc'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
		}
   ...
   // alert(str);
    ...
    //var tf = fso.CreateTextFile("d:\\test1.txt", true); 
   // tf.Write (str);
   // tf.Close();
...
dTree.prototype.setCheck=function(nodeIds){
    ...
    var name = this.obj;
  	...
  	for ( i = 0 ; i < j ; i++ ){
  	    ...
  	    if ( obj != null ){						
  	    	if ( this.isInIds(obj.name,tempIds))
  	  		{
  	            obj.checked = 0;				
 		        this.cc(i);
  	  		}
