 ...
 * 
 */
public class TowerTiltYearAnalysisAction extends
	...
	public String list() {
		...
		if (this.StartYearId == null) {
			this.StartYearId = cal.get(Calendar.YEAR) - 1;
		}
		...
		Page<Map<String, Object>> pages = this.towerTiltAnalysisService
				.getYearTowerTiltSamplings(sensor, beginTime, endTime,
	...
	public String chart() {
		...
		// 生成图形
		this.chart = JFreeChartFactory.createTimeSeriesChart("杆塔倾斜年数据分析曲线图",
	...
	public String exportExcel()
	{
		...
		List<Map<String, Object>> ttSamplings = this.towerTiltAnalysisService.getYearTowerTiltSamplings(sensor, beginTime, endTime);
		...
		String fname = String.format("%s-年数据",newSensor.getSensorCode());
		...
		try {
		    ...
		    boolean ex = this.towerTiltAnalysisService.exportExcel(ContextKeys.YEAR_EXPORT, newSensor, beginTime, endTime, ttSamplings, os,null,ContextKeys.NOTHING);
		    if (  ex  ){
