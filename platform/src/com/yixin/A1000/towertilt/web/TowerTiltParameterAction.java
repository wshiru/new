 ...
 * @author  
 */
public class TowerTiltParameterAction extends ActionSupport implements
		ServletRequestAware {
	...
	/** 杆塔倾斜参数参数 **/
	private TowerTiltParameter towerTiltParameter;
	 ...
	 * 设置 杆塔倾斜参数配置业务逻辑接口
	 * 
	 * @param towerTiltParameterService
	 *            杆塔倾斜参数配置业务逻辑接口
	 */
	 ...
	 */
	public void setTowerTiltParameter(TowerTiltParameter towerTiltParameter) {
		this.towerTiltParameter = towerTiltParameter;
	}
	 ...
	 * @return
	 */
	public TowerTiltParameter getTowerTiltParameter() {
		return towerTiltParameter;
	}
	...
	public String sensorParam() {
		...
		this.towerTiltParameter = towerTiltParameterService.getTowerTiltParameterBySensor(sensor);
		return SUCCESS;
	}
	...
	public String saveSensorParam() {
		...
		TowerTiltParameter tp =  towerTiltParameterService.getTowerTiltParameterBySensor(sensor);
		if(tp == null){
			tp = new TowerTiltParameter();
			tp.setSensor(sensor);
			tp.setAngleXAlarm1(towerTiltParameter.getAngleXAlarm1());
			tp.setAngleXAlarm2(towerTiltParameter.getAngleXAlarm2());
			tp.setAngleXAlarm3(towerTiltParameter.getAngleXAlarm3());
			tp.setAngleYAlarm1(towerTiltParameter.getAngleYAlarm1());
			tp.setAngleYAlarm2(towerTiltParameter.getAngleYAlarm2());
			tp.setAngleYAlarm3(towerTiltParameter.getAngleYAlarm3());
			towerTiltParameterService.addTowerTiltParameter(tp);
		}else{
			tp.setAngleXAlarm1(towerTiltParameter.getAngleXAlarm1());
			tp.setAngleXAlarm2(towerTiltParameter.getAngleXAlarm2());
			tp.setAngleXAlarm3(towerTiltParameter.getAngleXAlarm3());
			...
			tp.setAngleYAlarm2(towerTiltParameter.getAngleYAlarm2());
			tp.setAngleYAlarm3(towerTiltParameter.getAngleYAlarm3());
			towerTiltParameterService.editTowerTiltParameter(tp);
		}
	...
	public String downAlarmParam() {
		...
		boolean result = this.deviceSettingsService.downTowerTiltAlarmParam(sensor, alarmParam);		
