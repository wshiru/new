 ...
 * Project platform
 *
 * Classname WhiteMonsoonWarningContions.java
 * 
 * Version 1.0.0
 * 
 * Creator 
 ...
 * @author 
 */
public class WhiteMonsoonWarningConditions extends MonsoonWarningConditions{
	...
	static {
		if(null != warningConditionService && null !=warningDictService){
			warningDict = warningDictService.getWarningDictByName(WarningDictNames.WHITEMONSOON_WARNING_DICT_NAME);
			if(null == warningDict){
				/* 添加告警类型默认值 */
				warningDict = new WarningDict();
				warningDict.setCode(WarningDictCodes.WHITEMONSOON_WARNING_CODE);
				warningDict.setName(WarningDictNames.WHITEMONSOON_WARNING_DICT_NAME);
